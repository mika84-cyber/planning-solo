...
function describeAction(payload: Record<string, unknown>) {
  switch (payload.action) {
    case 'pin': return payload.message ? 'Message enregistré : il s’affiche sur l’accueil de tous les comptes invités.' : 'Message retiré de l’accueil.';
    case 'move-member': return `${String(payload.member)} déplacé·e dans le groupe ${String(payload.group)}.`;
    case 'remove-member': return `${String(payload.member)} retiré·e des trois groupes.`;
    case 'cancel-job': return 'Programmation annulée.';
    case 'reschedule-job': return 'Publication reprogrammée.';
    case 'restore': return 'Élément restauré.';
    case 'dismiss-alert': return 'Alerte effacée du suivi.';
    case 'delete-job': return 'Publication effacée.';
    case 'purge-trash': return 'Élément supprimé définitivement de la corbeille.';
    default: return 'Modification enregistrée.';
  }
}
const demoOverview = (): Overview => ({ pin: demoPin, trash: [{ id: 'demo-trash', title: 'Ancien formulaire de congés', kind: 'document', deletedAt: new Date(Date.now() - 2 * 86400000).toISOString() }], jobs: [{ id: 'demo-job', title: 'Planning d’octobre', at: new Date(Date.now() - 86400000).toISOString(), status: 'done' }], alerts: [{ id: 'demo-alert', title: 'Consignes exposition', documentId: 'demo-document', total: 5, seen: 3, sent: 5, emailsSent: 5, unseenIds: ['demo-a', 'demo-b'], createdAt: new Date(Date.now() - 86400000).toISOString() }], groups: [1, 2, 3].map(number => ({ number: number as 1 | 2 | 3, members: number === 1 ? ['Camille Exemple'] : number === 2 ? ['Alex Exemple'] : [] })) });
...
export function AdminToolsPanel({ demoMode, onClose, onPreview, onChanged }: { demoMode: boolean; onClose: () => void; onPreview: () => void; onChanged: () => void }) {
  ...
  async function act(payload: Record<string, unknown>) {
    ...
    try {
      if (demoMode) {
        ...
        if (payload.action === 'dismiss-alert') setData(current => current && { ...current, alerts: current.alerts.filter(item => item.id !== payload.id) });
        if (payload.action === 'delete-job') setData(current => current && { ...current, jobs: current.jobs.filter(item => item.id !== payload.id) });
        if (payload.action === 'purge-trash' || payload.action === 'restore') setData(current => current && { ...current, trash: current.trash.filter(item => item.id !== payload.id) });
      } else { await api(payload); setData(await api<Overview>()); }
      if (payload.action === 'remove-member') { setMember(''); setSearch(''); }
      setStatus(demoMode ? `${describeAction(payload)} (modification simulée dans cette démo)` : describeAction(payload)); onChanged(); window.dispatchEvent(new Event('admin-tools-updated'));
    } catch (cause) { setError(`La modification n’a pas pu être enregistrée : ${cause instanceof Error ? cause.message : 'erreur inconnue'}.`); } finally { setBusy(false); }
  }
  ...
  return createPortal(<dialog ref={dialog} className="admin-tools-dialog" aria-labelledby="admin-tools-title" onCancel={event => { event.preventDefault(); if (!busy) onClose(); }}>
    ...
    {data && <>
      <details><summary>Message temporaire sur l’accueil</summary><form onSubmit={event => { event.preventDefault(); void act({ action: 'pin', message, until: new Date(until).toISOString() }); }}>
      ...
      <details><summary>Corbeille ({data.trash.length})</summary><p>Documents, contacts et collègues récupérables pendant 30 jours. Les anciennes suppressions définitives ne peuvent pas être récupérées.</p>
        {data.trash.length ? data.trash.map(item => <article key={item.id}><button type="button" className="admin-dismiss" aria-label={`Supprimer définitivement ${item.title}`} title="Supprimer définitivement" disabled={busy} onClick={() => { if (window.confirm(`Supprimer définitivement « ${item.title} » ? Il ne pourra plus être restauré.`)) void act({ action: 'purge-trash', id: item.id }); }}>×</button><strong>{item.title}</strong><small>{item.kind === 'contact' ? 'Contact' : item.kind === 'group-member' ? 'Collègue retiré des groupes' : 'Document'} · Supprimé le {formatDate(item.deletedAt)}</small><button type="button" disabled={busy} onClick={() => void act({ action: 'restore', id: item.id })}>Restaurer</button></article>) : <p>La corbeille est vide.</p>}
      </details>
      <details><summary>Composition des groupes</summary><p>{data.groups.map(item => `Groupe ${item.number} : ${item.members.length}`).join(' · ')}</p><p>Modifie l’annuaire de l’équipe. Le cycle personnel de planning reste réglé par chaque collègue.</p>
        <div className="admin-member-picker"><label>Rechercher un collègue<input type="search" role="combobox" autoComplete="off" placeholder="Prénom ou nom" aria-autocomplete="list" aria-controls="admin-member-suggestions" aria-expanded={suggestionsOpen && search.trim() !== '' && search !== member} value={search} onFocus={() => { window.clearTimeout(blurTimer.current); setSuggestionsOpen(true); }} onBlur={() => { blurTimer.current = window.setTimeout(() => setSuggestionsOpen(false), 150); }} onChange={event => { window.clearTimeout(blurTimer.current); setSearch(event.target.value); setMember(''); setSuggestionsOpen(true); }} /></label>
          {suggestionsOpen && search.trim() && search !== member ? <ul id="admin-member-suggestions" className="admin-member-suggestions" role="listbox" aria-label="Collègues proposés">{data.groups.flatMap(item => item.members.filter(name => matchesSearch(name, search)).map(name => <li key={name}><button type="button" role="option" aria-selected={name === member} onMouseDown={event => event.preventDefault()} onClick={() => { setMember(name); setSearch(name); setSuggestionsOpen(false); }}><strong>{name}</strong><small>Groupe {item.number}</small></button></li>)).slice(0, 8)}</ul> : null}
        </div>
        <form onSubmit={event => { event.preventDefault(); void act({ action: 'move-member', member, group }); }}>{member && <p className="admin-member-selected">Collègue choisi : <strong>{member}</strong> · Groupe {data.groups.find(item => item.members.includes(member))?.number}</p>}
    ...
    {(error || status) && <div className={`admin-toast ${error ? 'is-error' : 'is-success'}`} role={error ? 'alert' : 'status'}><span aria-hidden="true" /><p>{error || status}</p><button type="button" aria-label="Fermer le message" onClick={() => { setError(''); setStatus(''); }}>×</button></div>}
