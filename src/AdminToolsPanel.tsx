  ...
  return createPortal(<dialog ref={dialog} className="admin-tools-dialog" aria-labelledby="admin-tools-title" onCancel={event => { event.preventDefault(); if (!busy) onClose(); }}>
    ...
    {data && <>
      <details open><summary>Message temporaire sur l’accueil</summary><form onSubmit={event => { event.preventDefault(); void act({ action: 'pin', message, until: new Date(until).toISOString() }); }}>
        ...
        <button disabled={busy || !message.trim() || !until}>Enregistrer le message</button>{data.pin && <button type="button" disabled={busy} onClick={() => void act({ action: 'pin', message: '' })}>Retirer le message</button>}
      </form></details>
      ...
      <details><summary>Suivi des alertes ({data.alerts.length})</summary><p>« Vue » signifie que la popup a été affichée, pas que le document a été lu. Le suivi commence avec les nouveaux partages.</p>
        {data.alerts.length ? data.alerts.map(alert => <article key={alert.id}><strong>{alert.title}</strong><p>{formatDate(alert.createdAt)}</p><p>Vue par {alert.seen} personnes sur {alert.total}</p><small>{alert.sent} alertes déposées · {alert.emailsSent} e-mails envoyés</small>{alert.unseenIds.length > 0 && <button disabled={busy} onClick={() => setConfirmation(alert)}>Relancer les {alert.unseenIds.length} personnes restantes</button>}</article>) : <p>Aucune alerte suivie pour le moment.</p>}
        {confirmation && <section><p>Envoyer une nouvelle alerte et un e-mail aux {confirmation.unseenIds.length} destinataires qui n’ont pas vu « {confirmation.title} » ?</p><button disabled={busy} onClick={() => void remind()}>Confirmer la relance</button><button disabled={busy} onClick={() => setConfirmation(null)}>Annuler</button></section>}
      </details>
      <details><summary>Corbeille ({data.trash.length})</summary><p>Documents, contacts et collègues récupérables pendant 30 jours. Les anciennes suppressions définitives ne peuvent pas être récupérées.</p>
        {data.trash.length ? data.trash.map(item => <article key={item.id}><strong>{item.title}</strong><small>{item.kind === 'contact' ? 'Contact' : item.kind === 'group-member' ? 'Collègue retiré des groupes' : 'Document'} · Supprimé le {formatDate(item.deletedAt)}</small><button disabled={busy} onClick={() => void act({ action: 'restore', id: item.id })}>Restaurer</button></article>) : <p>La corbeille est vide.</p>}
      </details>
